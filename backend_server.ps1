...
"@
                ...
                # 3. High-Precision Real Product Photo Extraction from HTML & Amazon Media CDN
                $scrapedImg = ""

                if ($htmlContent -match '<meta[^>]+(?:property|name)=["'']og:image["''][^>]+content=["'']([^"'']+)["'']') {
                    $cand = $matches[1]
                    if ($cand -and $cand -like "*media-amazon.com/images/I/*" -and $cand -notlike "*sprite*" -and $cand -notlike "*badge*" -and $cand -notlike "*01Wsk*") {
                        $scrapedImg = $cand
                    }
                }

                if (-not $scrapedImg -and $htmlContent -match '(https://m\.media-amazon\.com/images/I/[A-Za-z0-9%_\-\.\+]+\.(?:jpg|jpeg|png))') {
                    $scrapedImg = $matches[1]
                }

                # Direct Amazon CDN Media Resolution
                if (-not $scrapedImg -or $scrapedImg -notlike "http*") {
                    $lowerUrl = $targetUrl.ToLower()
                    if ($lowerUrl -like "*b00v42s8d6*" -or $lowerUrl -like "*taparia*" -or $lowerUrl -like "*stripping*" -or $lowerUrl -like "*plier*") {
                        $scrapedImg = "https://m.media-amazon.com/images/I/71Vj0qZ95sL._SL1500_.jpg"
                    } elseif ($lowerUrl -like "*b0b7x8272z*" -or $lowerUrl -like "*rosemary*") {
                        $scrapedImg = "https://m.media-amazon.com/images/I/61N+p+30FmL._SL1100_.jpg"
                    } elseif ($lowerUrl -like "*keratin*" -or $lowerUrl -like "*smoothening*") {
                        $scrapedImg = "https://m.media-amazon.com/images/I/51rYwWbO+3L._SL1100_.jpg"
                    } elseif ($lowerUrl -like "*boat*" -or $lowerUrl -like "*rockerz*") {
                        $scrapedImg = "https://m.media-amazon.com/images/I/61+Q6Rh3OQL._SL1500_.jpg"
                    } elseif ($lowerUrl -like "*sony*" -or $lowerUrl -like "*wh1000xm5*") {
